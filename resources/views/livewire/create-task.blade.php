<div class="max-w-3xl mx-auto py-8">
    <h1 class="text-2xl font-semibold mb-6">Tambah Tugas Baru</h1>
    ...
    <form wire:submit.prevent="save" class="space-y-4" enctype="multipart/form-data">
        ...
        <div>
            <label class="block text-sm font-medium">Lampiran (maks 5MB/file, JPG/PNG/PDF)</label>
            <input type="file" wire:model="attachments" multiple class="mt-1 w-full border rounded p-2">
            @error('attachments.*')<p class="text-sm text-red-600">{{ $message }}</p>@enderror
            <div class="text-xs text-gray-500 mt-1">Anda dapat memilih beberapa file.</div>
        </div>

        <div>
            <label class="block text-sm font-medium">Catatan</label>
            <textarea wire:model.defer="note_content" rows="4" class="mt-1 w-full border rounded p-2"></textarea>
            @error('note_content')<p class="text-sm text-red-600">{{ $message }}</p>@enderror
        </div>

        <div class="flex items-center gap-3">
            <button type="submit" class="px-4 py-2 rounded bg-indigo-600 text-white">Simpan</button>
            <a href="{{ route('dashboard') }}" class="px-4 py-2 rounded border">Batal</a>
        </div>
    </form>
</div>
